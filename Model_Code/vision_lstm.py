...
class VisionLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            x = x+block(x)
            if i==3:
                output.append(x.reshape(x.shape[0],self.dim,32,32))
            elif i==5:
                output.append(x.reshape(x.shape[0],self.dim,32,32))
            elif i==6:
                x = x.reshape(x.shape[0],self.dim,32,32)
                output.append(x)
                return x
            i = i+1
        x = x.reshape(x.shape[0],self.dim,32,32)
...
def vil_tiny(pretrained=True, **kwargs):
    model = VisionLSTM(dim=192, depth=24, legacy_norm=True, **kwargs)
...
def vil_tiny_e400(pretrained=True, **kwargs):
    model = VisionLSTM(dim=192, depth=24, legacy_norm=True, **kwargs)
...
def vil_tinyplus(pretrained=True, **kwargs):
    model = VisionLSTM(dim=192, depth=29, legacy_norm=True, **kwargs)
...
def vil_tinyplus_e400(pretrained=True, **kwargs):
    model = VisionLSTM(dim=192, depth=29, legacy_norm=True)
...
def vil_tinyplus_stride8(pretrained=True, **kwargs):
    model = VisionLSTM(dim=192, depth=29, stride=8, legacy_norm=True, **kwargs)
    url = "https://ml.jku.at/research/vision_lstm/download/vil_tinyplus16_e800_in1k.th"
    preprocess = "v1"
    if pretrained:
        assert url is not None, f"pretrained=True but no url found"
...
def vil_small(pretrained=True, **kwargs):
    model = VisionLSTM(dim=384, depth=24, legacy_norm=True, **kwargs)
...
def vil_smallplus(pretrained=True, **kwargs):
    model = VisionLSTM(dim=384, depth=26, legacy_norm=True, **kwargs)
...
def vil_smallplus_stride8(pretrained=True, **kwargs):
    model = VisionLSTM(dim=384, depth=26, stride=8, legacy_norm=True, **kwargs)
...
def vil_base(pretrained=True, **kwargs):
    model = VisionLSTM(dim=768, depth=24, legacy_norm=True, **kwargs)
...
def vil_base_stride8(pretrained=True, **kwargs):
    model = VisionLSTM(dim=768, depth=24, stride=8, legacy_norm=True, **kwargs)
